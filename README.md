...
- **插件驱动**：功能模块化，易于扩展和维护
- **多平台**：QQ（NapCat/LLBot WebSocket/HTTP，OneBot v11）+ 飞书（官方 SDK 长连接/Webhook）+ Telegram（Bot API 长轮询，无需公网地址）+ Discord（discordgo，Gateway WebSocket，无需公网地址）+ 微信（iLink bot，HTTP 长轮询，扫码登录，无需公网地址）等；新增平台只需实现一个适配器并注册，框架核心零改动
