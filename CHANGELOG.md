...
- 面板 Token 统计页支持时间维度筛选：页面顶部新增 全部 / 今日 / 昨日 / 近 7 天 / 近 30 天 / 本月 切换，统计卡、分来源每日序列、来源 / 会话类型 / 状态拆分、24 小时分布与目标排行均按所选窗口聚合；`GET /api/tokenstats/detail` 相应新增 `range` 查询参数（服务端直接映射为日志的 Start/End 过滤），响应新增 `range` 字段，每日序列长度随窗口变化（all 时仍为最近 30 天）
