...
1. **平台接入** —— 勾选要启用的平台：QQ(NapCat，默认勾选，填写连接地址与 Access Token) 和/或 QQ 官方（填写 AppID / AppSecret，可选沙箱环境）和/或 飞书（填写 App ID / Secret 与事件订阅方式）和/或 Telegram（填写 Bot Token，可选代理），以及管理员 ID（QQ 号或带前缀的 ID）
