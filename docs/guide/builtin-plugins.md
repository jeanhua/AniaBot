...
默认关闭。开启后，`approval.tools` 列出的工具（默认 `file,config_set`）每次执行前都会向会话发送一条确认消息，**消息发送者或机器人管理员**在超时时间（默认 120 秒）内回复「允许」或「拒绝」决定放行与否，超时自动拒绝；子代理 / 定时任务发起的审批仅管理员可批。审批期间正常聊天不受影响。

bash 工具使用独立的命令级三段式权限（无需列入 `approval.tools`）：

1. 命中**黑名单**正则 → 直接拒绝；
2. 命中**白名单**正则 → 直接放行；
3. 两者都不命中（包括两份名单都为空）→ 发起上述人工审批，批准后执行；审批未启用时拒绝并提示配置方法。
