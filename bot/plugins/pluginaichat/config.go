...
type queryLogConfig struct {
	...
	MaxResultRunes int  `cfg:"max_result_runes" label:"工具结果截断上限(字符)" group:"AI 对话 · 查询日志" help:"工具执行结果超过该字符数时截断保存；0 表示不限制。仅此字段限长（网页抓取、命令执行等外部输出可能很大），用户输入、工具参数、最终回复始终完整记录" default:"1000"`
}
