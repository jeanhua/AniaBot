...
// 供事后查询与上报（如定时任务的触发 / 成功 / 超时 / 失败记录）。持久化存储固定
// 为 SQL 后端（sqlite/mysql），日志每条一行（ania_task_log 表，过滤条件下推
// WHERE、容量淘汰走范围删除）；探测或建表失败时降级为禁用后端（写入静默丢弃），
// 仅记录错误日志。日志 ID 为自增序号的 base36。
package tasklog
...
// （sqlite/mysql），日志走 ania_task_log 行级存储；探测或建表失败时降级为
